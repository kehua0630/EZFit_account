				...
				<span id="c" class="col-4 m-0 align-self-center text-right member">
					<c:if test="${ ! empty LoginOK }">
						<img class="img"
							...
							<c:when test='${(empty FB) and (empty mempic)}'>src='${pageContext.request.contextPath}/index/img/nav/logo-g.svg'</c:when>
						...
						test="${empty LoginOK}">
						<a
							onclick="window.location.href = '${pageContext.request.contextPath}/login/login.jsp'"
							id="c" class="col-4 m-0 align-self-center text-right"> 會員登入 </a>
					</c:if> <c:if test="${ ! empty LoginOK }">

						<a
							onclick="window.location.href = '${pageContext.request.contextPath}/memberPage/memberPage.jsp'"
							id="c" class="col-4 m-0 align-self-center text-right">${LoginOK.name}</a>
					</c:if> <span>&nbsp; | &nbsp;</span> <c:if test="${empty LoginOK}">
						<a
							onclick="window.location.href = '${pageContext.request.contextPath}/createAccount/createAccount.jsp'"
							id="c" class="col-4 m-0 align-self-center text-right">註冊會員</a>
					</c:if> <c:if test="${ ! empty LoginOK }">
						<a
							onclick="window.location.href = '${pageContext.request.contextPath}/login/logout.jsp'">
							登出 </a>
				...
				<span id="d" class="col-3 m-0 align-self-center text-right member">
					<c:if test="${ ! empty LoginOK }">
						<img class="img"
							...
							<c:choose>
							<c:when test='${! empty FB }'>src='${LoginOK.memberImage}'</c:when>
							<c:when test='${(empty FB) and (empty mempic)}'>src='${pageContext.request.contextPath}/index/img/nav/logo-w.svg'</c:when>
							<c:otherwise> src='${pageContext.request.contextPath}/MemberImage/${Id}.jpg' </c:otherwise>  
							</c:choose>>
					</c:if> <span id="notification"></span> <span id="cart"></span> <c:if
						test="${empty LoginOK}">
